...
func TestUpdateFinancialProfile_MoreCases(t *testing.T) {
	tests := []struct {
		name        string
		input       UpdateFinancialProfileInput
		setupMock   func(*MockFinancialPlanRepository)
		...
		},
		{
			...
			setupMock: func(m *MockFinancialPlanRepository) {
				...
				m.On("FindByUserID", mock.Anything, entities.UserID("user-002")).Return(plan, nil)
				m.On("Update", mock.Anything, mock.AnythingOfType("*aggregates.FinancialPlan")).Return(errors.New("DB更新エラー"))
			},
			expectError: true,
			errContains: "財務計画の保存に失敗しました",
		},
	}
