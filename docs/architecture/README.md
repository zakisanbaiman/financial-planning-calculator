...
### [ドメイン境界図 (Domain Diagram)](./DOMAIN_DIAGRAM.md)
ドメイン境界・依存関係と設計意図を可視化した図です。
...
- 財務計画・認証ドメインの集約境界
- ユースケースとリポジトリの依存マップ
- Infrastructure LayerのDecoratorパターン（Redisキャッシュ）
- 典型的なデータフロー（シーケンス図）
...
- ドメインモデル（エンティティ、値オブジェクト、集約）の属性・メソッド詳細
- ユースケース
- リポジトリインターフェース
- ドメインサービス
...
2. [ドメイン境界図](./DOMAIN_DIAGRAM.md) - ドメイン境界・依存関係の把握
3. [クラス図](./CLASS_DIAGRAM.md) - ドメインモデルの属性・メソッド詳細
4. [ER図](./ER_DIAGRAM.md) - データベーススキーマの理解
5. [システム構成図](./SYSTEM_ARCHITECTURE.md) - APIフローの理解
